...
describe('test validity of email input', () => {
  ...
  it('should return a 422 if the email is not valid', async () => {
    await request(app)
      ...
      .expect(422);
    await request(app).post('/api/auth/signup').send({ email: '@test.com', password }).expect(422);
  });
...
describe('test validity of password input', () => {
  ...
  it('should return 422 if the password does not contain a number', async () => {
    await request(app).post('/api/auth/signup').send({ email, password: 'Validvalid' }).expect(422);
  });
